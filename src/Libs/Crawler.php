...
class Crawler {
	...
	protected function setCurlOptions($post = False,
	 ...
	 /**
		...
		curl_setopt($this->_curlHandle, 
		            CURLOPT_URL, 
		            empty($url) ? App::$settings['uber_login_url'] : $url);
		...
		curl_setopt($this->_curlHandle, 
		            CURLOPT_TIMEOUT, 
		            App::$settings['curl_timeout']);
		/**
		 * Session Data Storage File
		 */
		curl_setopt($this->_curlHandle, 
		            CURLOPT_COOKIEJAR, 
		            App::$settings['cookies_storage_file']);
		/**
		 * Session Data Storage File
		 */
	  curl_setopt($this->_curlHandle, 
	...
	protected function grabLoginForm() {
		...
		if (!curl_errno($this->_curlHandle)) {
			...
			if (empty($this->_csrf_token)) {
				$this->exception("Grabbing CSRF Token Failed - {$errorMessage}", 
			                   "FATAL");
			}
	...
	protected function getData() {
		...
		if (!curl_errno($this->_curlHandle)) {
			...
			// Failed to Login
			$errorMessage = curl_error($this->_curlHandle);
			$this->exception("Login Attempt Failed! - {$errorMessage}", 
			                 "FATAL");
	...
	protected function storeIntoFile($data, 
		...
		// don't exist already
		$this->makedirs(App::$settings['data_storage_dir']);
	...
	protected function buildStorageFilePath($pageNumb) {
		...
		return join("/", [App::$settings['data_storage_dir'],
		     			 $fileName
		   ]);
	 ...
	 * @return [type] [description]
	 */
	protected function exception($message, $type = 'FATAL') {
